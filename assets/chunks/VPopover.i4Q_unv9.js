...
import { applyTransform, levitate, on, trap } from '../shared';\r
...
\r
  watchEffect(onCleanup => {\r
    ...
    onCleanup(\r
      trap($pop, thief => {\r
        if ($pop.contains(thief)) return false;\r
...
\r
        if ($pop.contains(thief)) return;\r
...
\r
  bagEffect(bag => {\r
    ...
    if (!$pop) return;\r
    bag(on($pop).keydown.exact.Escape.prevent(() => (open.value = false)));\r
\r

...
\r
const props = defineProps<VPopoverProps>();\r
\r
...
\r
const anchor = forwardRef(useTemplateRef('_anchor'));\r
const popper = forwardRef(useTemplateRef('_popper'));\r
...
\r
const Popover = defineComponent(\r
  () => () =>\r
...
<template>\r
  ...
  <Teleport to="body" :disabled="!(pop.open || popper)">\r
    <i-edge v-if="pop.open" />\r
    <!-- prettier-ignore -->\r
    <Transition v-if="animated">\r
      <component ref="_popper" v-if="pop.open" :is="Popover" />\r
    </Transition>\r
    <template v-else>\r
      <component ref="_popper" v-if="pop.open" :is="Popover" />\r
    </template>\r
